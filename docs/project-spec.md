...
- Keep environment setup to one-command lifecycle actions through Make targets.
...
- ArgoCD can sync baseline apps from the repository with no manual per-app deploy steps.
...
- Docker runtime, k3d, kubectl, and make are available on target host.
...
- Mitigation: Keep bootstrap manifest review as first checklist item.
...
- In scope: k3d cluster setup, ArgoCD install, modular app manifests, ingress/domain strategy, local validation workflow.
- Out of scope: production hardening, enterprise secret manager integration, multi-cloud platform rollout.

# Functional Requirements

- FR-001: Repository shall provide make targets for up/down/status/password operations.
